    ...
    <div className={editing ? "grid gap-6 lg:grid-cols-[1fr_420px]" : "grid gap-6"}>
      <section>
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {categories.map((category) => (
            <article key={category.id || category.slug} className="glass-panel overflow-hidden rounded-[1.8rem]">
              <div className="relative aspect-[4/3]">
                <Image src={category.imageUrl} alt={category.name} fill className="object-cover" />
              </div>
              <div className="p-5">
                <h2 className="font-serif text-3xl text-ivory">{category.name}</h2>
                <p className="mt-2 text-sm leading-6 text-ivory/62">{category.description}</p>
                <div className="mt-5 flex gap-3">
                  <button className="text-sm text-champagne" onClick={() => setEditing(category)}>
                    Edit
                  </button>
                  <button className="text-sm text-red-300" onClick={() => remove(category)}>
                    Delete
                  </button>
                </div>
              </div>
      ...
      {editing && (
        <CategoryForm
          ...
          onChangeName={(name) =>
            setEditing((current) => ({ ...(current ?? emptyCategory), name, slug: slugify(name) }))
          }
          onSaved={(saved) => {
...
function CategoryForm({
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(
      category.id ? `/api/admin/categories?id=${category.id}` : "/api/admin/categories",
      {
  ...
  return (
    <form onSubmit={submit} className="glass-panel grid h-fit gap-4 rounded-[1.8rem] p-6">
      ...
      />
      <input name="slug" className={fieldClass} placeholder="Slug" value={category.slug} readOnly />
      <textarea
        name="description"
